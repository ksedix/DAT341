    ...
    def fit(self, X, Y):
        ...
        for i in range(self.n_iter):
            for x, y in zip(X, Ye):
                ...
                if y*score <= 0:
                    #subtract the feature if the predicted was positive but the actual was negative. Add the feature if the predicted was negative but the real was positive.
                    self.w += y*x
    ...
    def fit(self, X, Y):
        ...
        for t in range(1,self.n_iter+1):
            for x, y in zip(X, Ye):
                ...
                # If there was an error, update the weights.
                if y*score < 1:
                    self.w = self.w - (eta * regularizer * self.w) + (eta * y * x)
                else:
                    self.w = self.w - (eta * regularizer * self.w)
